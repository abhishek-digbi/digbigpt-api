...
class _CapturingSummaryStub:
    def __init__(self):
        self.formatted_responses = None

    async def ask(self, ctx):
        self.formatted_responses = copy.deepcopy(ctx.data.get("formatted_responses"))
        return {"status": "ok", "message": "summary", "meta": {}}
...
@pytest.mark.asyncio
async def test_summarize_response_strips_agent_meta_before_summary(monkeypatch):
    monkeypatch.setattr("utils.env_loader.get_ask_digbi_assistant_id", lambda: "assist")
    agent, dummy = setup_agent(monkeypatch)
    capturing_summary = _CapturingSummaryStub()
    agent.summarizer_agent = capturing_summary
    ...
    assert capturing_summary.formatted_responses is not None
    assert capturing_summary.formatted_responses[0].get("meta") is None
    assert capturing_summary.formatted_responses[0]["agent"] == "support_agent"
