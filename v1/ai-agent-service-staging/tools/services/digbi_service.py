...
# Import env_loader with proper path handling
try:
    ...
    # Go up from tools/services/ to ai-agent-service-staging/
    staging_dir = current_file.parent.parent.parent
    if staging_dir.name != "ai-agent-service-staging":
        staging_dir = current_file.parent.parent.parent.parent
    if str(staging_dir) not in sys.path:
        sys.path.insert(0, str(staging_dir))
    try:
        from utils.env_loader import *
    except ImportError:
        # Create empty fallback - these functions won't be available
        pass
...
# Get barcodes scanned URL function if available
try:
